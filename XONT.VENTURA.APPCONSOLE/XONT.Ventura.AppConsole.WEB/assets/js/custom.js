...
$(function () {
    ...
    $("#btnSystemVersionInfo").click(
        function ()
        {
            $("#systemVersionInfo").show();
            var url = 'SystemDiagnostics.aspx?TaskID=ZYMNT02&TaskType=V3';
            $('#ifrmSystemVersionInfo').attr('src', url)
        });
    $("#systemVersionInfoClose").on('click', function (c) {
        PageMethods.ClearSystemInfoSessions();
        $("#systemVersionInfo").fadeOut('slow', function (c) {
        });
    });
    //V2036Adding end

    //<!-- System Version Info Modal Popup -->
    ...
    $("#crm").click(function () {
        //$("#crm_Modal").show()
        window.open('http://crm.xontworld.com/xont_crm/', '_blank');
    });;
    ...
    $("#delete_notification").on('click', function (c) {
        $("#remove_notification").show();
        $("#delete_notification").hide();
        $("#cancel_notification").hide();
        $(".notification_checkbox").hide();
        $(".role_notification").prop('disabled', false);
        $("#check_all").hide();
        $("#uncheck_all").hide();
        notification_Array = [];

        if ($('.notification_section :checkbox:checked').length > 0) {
            $('.notification_section input:checkbox:checked').each(function () {
                var tabId = $(this).parent().siblings('.notificationID').children().attr("value");

                notification_Array.push(tabId);
                $(this).parent().parent().remove();
            });
            PageMethods.deleteNotifications(notification_Array);
        }


    });

    $("#remove_notification").on('click', function (c) {
        $("#remove_notification").hide();
        $("#delete_notification").show();
        $("#cancel_notification").show();
        $(".notification_checkbox").show();
        $(".role_notification").prop('disabled', true);
        $("#check_all").show();
        $("#uncheck_all").hide();
    });

    $("#cancel_notification").on('click', function (c) {
        $("#remove_notification").show();
        $("#delete_notification").hide();
        $("#cancel_notification").hide();
        $(".notification_checkbox").hide();
    ...
    $("#back_notification").on('click', function (c) {
    ...
    $("#check_all").on('click', function (c) {
        $("#check_all").hide();
        $("#uncheck_all").show();
        $('.notification_checkbox').children().prop('checked', true);
    });
    $("#uncheck_all").on('click', function (c) {
        $("#check_all").show();
        $("#uncheck_all").hide();
        $('.notification_checkbox').children().prop('checked', false);
