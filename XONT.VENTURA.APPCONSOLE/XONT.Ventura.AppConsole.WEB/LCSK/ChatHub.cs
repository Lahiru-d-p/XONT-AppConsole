...
namespace XONT.Ventura.AppConsole.LCSK
{
    public class ChatHub : Hub
    {
        ...
        */
        ...
        public void LogVisit(string page, string referrer, string city, string region, string country, string existingChatId, string userId)
        {
            if (Agents == null)
                Agents = new ConcurrentDictionary<string, Agent>();
            ...
            foreach (var agent in Agents)
            {
                ...
                Clients.Client(agent.Value.Id).newVisit(page, referrer, cityDisplayName, countryDisplayName, chatWith, Context.ConnectionId, userId);
            }
        }

        public void RequestChat(string message,string c_user)
        {
            ...
            Clients.Client(lessBuzy.Id).addMessage(Context.ConnectionId, c_user, message);
        ...
        public void Send(string data, string ch_user)
        {
            ...
            if (ChatSessions.ContainsKey(Context.ConnectionId))
            {
                ...
                Clients.Client(opId).addMessage(Context.ConnectionId, ch_user, data);
            }
            else
            {
                ...
                Clients.Client(lessBuzy.Id).addMessage(Context.ConnectionId, ch_user, data);
            }
        }
        ...
        public void SetConfig(string token, string adminPass, string agentPass)
        {
            ...
            if (shouldSave)
            {
              // string configPath = HttpContext.Current.Server.MapPath("../App_Data/" + CONFIG_FILE);
                  string configPath = Path.Combine(HttpRuntime.AppDomainAppPath, "App_Data/" + CONFIG_FILE);
                File.WriteAllText(
                    configPath,
                    ToHash(adminPass) + "\n" + ToHash(agentPass));
