...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript" language="javascript">
        ...
        function defaultBusinessUnit(role) {

            $('.role_checkbox > input').prop('checked', false);
            $('.role_checkbox').each(function () {
                var t = $(this).parent().next().attr('value');
                if (t.trim() == role.trim()) {
                    $(this).children('input').prop('checked', true);
                    $(this).children('input').prop('disabled', true);
                    $(this).parent().find('.role_bookmark').addClass('current_role');
                    $(this).parent().find('.role_bookmark').parent().parent().addClass('role_highlight');
                }
            });
        ...
        function MyMethod_Result(ResultString) {
            ...
            console.log('My Method Result Bro');
            var dsrp2 = new Array();
            dsrp2 = ResultString;
            //var url = dsrp2[0] + "/" + dsrp2[0] + ".aspx";V2039Removed
            var url = dsrp2[4];//V2039Added
            var desc = dsrp2[1];
            var taskid = dsrp2[0];
            var tasktype = dsrp2[2];

            var username = dsrp2[3];

            LoadTask(url, desc, taskid, tasktype, username);
        }
        function getTaskDetails(taskcode) {
            PageMethods.getNotificationTask(taskcode, MyMethod_Result);
        }

        $(document).on('click', ' .role_notification', function () {
            var tabId = $(this).parent().parent().siblings(".notificationType").children().attr("value");
            if (tabId == 'T') {
                var taskcode = $(this).parent().parent().siblings(".notificationTaskCode").children().attr("value").trim();
                getTaskDetails(taskcode);
            }
            else {
                document.getElementById("notification_sideNav").style.width = "360px";
                $(".notification_section").hide();
                $(".notification_message_section").show();
                $("#remove_notification").hide();
                $("#back_notification").show();
                var message = $(this).parent().parent().siblings(".notificationMessage").children().attr("value");
                $(".message_section").text(message);
                $("#check_all").hide();
                $("#uncheck_all").hide();
            }

            var recId = $(this).parent().parent().siblings(".notificationID").children().attr("value");

            $(this).parent().parent().css("color", "rgb(247, 185, 120) !important");
            $(this).css("color", "rgb(247, 185, 120) !important");
            $(this).parent().parent().find('i').removeClass("fa-envelope");
            $(this).parent().parent().find('i').addClass("fa-check-circle");

            PageMethods.executeNotification(recId);
            return false;
        });
        ...
        function openNav() {
            document.getElementById("notification_sideNav").style.width = "250px";


            $('.notificationStatus').each(function () {
                var tabId = $(this).children().attr("value");
                if (tabId == 3) {
                    $(this).siblings('a').css("color", "rgb(247, 185, 120) !important");
                    $(this).siblings('a').find('i').removeClass("fa-envelope");
                    $(this).siblings('a').find('i').addClass("fa-check-circle");
                    $(this).siblings('a').find(".role_notification").css("color", "rgb(247, 185, 120) !important");
                }
            });

            $("#UnreadMessages").hide();
            PageMethods.OpenNotification();
       ...
       "<span class='fa fa-chevron-circle-right'></span>"
            ...
            $(document).mouseup(function (e) {
                ...
                //V2017
                var container = $("#notification_sideNav");

                if (!container.is(e.target) // if the target of the click isn't the container...
                    && container.has(e.target).length === 0) // ... nor a descendant of the container
                {
                    document.getElementById("notification_sideNav").style.width = "0";
                    $(".notification_section").show();
                    $(".notification_message_section").hide();
                    $("#remove_notification").show();
                    $("#back_notification").hide();
                    $("#check_all").hide();
                    $("#uncheck_all").hide();
                }
    ...
    <style>
        ...
        .sidenav {
            ...
            background-color: rgb(30, 81, 125) !important;
            overflow-x: hidden;
            transition: 0.5s;
            width: 255px;
            height: auto;
...
<body class="ovh" onbeforeunload="HandleClose();" onload="autoCSS();" style="-ms-overflow-y: hidden;">
    <form id="form1" runat="server">
        ...
        <div id="settings_Modal" class="modalDialog2 fade in" style="display: none; position: relative; height: 100%;">
            <div>
                <div class="s-content" style="height: 390px; width: 75%">
                    <div class="s-header">
                        <h4 class="s-title" style="font-weight: bold;">Settings </h4>
                        <a id="setting_close" class="fa fa-close" style="float: right; cursor: pointer; margin-top: 10px; margin-right: 10px; color: #fff; margin-top: -30px; text-decoration: none;"></a>
                    </div>
                            ...
                            <div style="margin-top: 10px;">
                                <div style="position: relative; width: 200px; top: 4px; height: 200px; background: rgba(128, 128, 128, 0.27) none repeat scroll 0% 0%; padding: 10px; margin: 0px 5px 5px; box-shadow: 0px 0px 15px rgba(15, 11, 11, 0.5);">
                                    ...
                                    <div>
                                        ...
                                        <span style="float: right; padding: 10px;">Default Avatar</span>
    ...
    <div>	
        ...
        <div id="mail_Modal" class="modalDialog2 fade in" style="display: none; position: relative; height: 100%;">
            <div>
                ...
                <iframe id="Iframe1" style="border: 0px; padding: 0px; border-style: none; top: 0; bottom: 0; left: 0; right: 0; position: absolute; border-width: 0px; margin: auto; width: 450px; height: 380px"></iframe>
        ...
        <div id="multimedia_Modal" class="modalDialog2 fade in" style="display: none; position: relative; height: 100%;">
            <div>
                ...
                <iframe id="Iframe1Multimeadia" style="border: 0px; padding: 0px; border-style: none; top: 0; bottom: 0; left: 0; right: 0; position: absolute; border-width: 0px; margin: auto; width: 100%; height: 100%;"></iframe>
        ...
        <div id="about_Modal" class="modalDialog2 fade in" style="display: none; position: relative; height: 100%;">
            <div>
                <div class="s-content" style="height: 335px;">
                    ...
                    <div class="s-header">
                        <h4 class="s-title" style="font-weight: bold;">About Ventura</h4>
                    </div>
                    <div class="s-body">
                    </div>
                    <div class="s-footer">
            ...
            <div>
                <div class="s-content" style="height: 145px;">
                    <div class="s-header" style="padding: 5px !important;">
                        <h4 class="s-title" style="font-weight: bold;">Confirm</h4>
                    </div>
                    <div class="s-body">
        ...
        <div id="max_tab" class="modalDialog2 fade in" style="display: none; position: relative; height: 100%;">
            <div>
                <div class="s-content" style="height: 175px; width: 500px;">
                    <div class="s-header" style="padding: 5px !important;">
                        <h4 class="s-title" style="font-weight: bold;">Alert!</h4>
                    </div>
                    <div class="s-body">
        ...
        <div id="crm_Modal" class="modalDialog2 fade in" style="display: none; position: relative; height: 100%;">
            <div>
                <div class="s-content" style="height: 570px; width: 95%;">
                    <div class="s-header">
                        <h4 class="s-title" style="font-weight: bold;">CRM <a id="crm_close" class="fa fa-close" style="float: right; cursor: pointer; margin-top: 10px; margin-right: 10px; color: #fff;"></a></h4>
                    </div>
                    <div class="s-body">
                        <iframe src="http://x-ontcrm.xontworld.com" style="width: 100%; height: 492px; background-color: #fff; margin-top: 10px"></iframe>
                    </div>
            ...
            <div>
                <div class="s-content" style="height: 570px; width: 95%;">
                    <div class="s-header">
                        ...
                        <a style="font-size: 20px; float: right; text-decoration: none; cursor: pointer; margin-top: -20px;" class="fa fa-close" id="help_close"></a>
                    </div>
                    <div class="s-body">
                        <iframe src="HelpFiles/HelpContents.aspx" style="width: 100%; height: 492px; background-color: #fff; margin-top: 10px"></iframe>
        ...
        <!-- Version Info modal -->
        <div id="systemVersionInfo" class="modalDialog2 fade in" style="display: none; position: relative; height: 100%;">
            <div>
                <div class="s-content" style="height: 570px; width: 95%;">
                    <div class="s-header">
                        <h4 class="s-title" style="font-weight: bold;">System Version Infomation </h4>
                        <a style="font-size: 20px; float: right; text-decoration: none; cursor: pointer; margin-top: -29px;color:white;" class="fa fa-close" id="systemVersionInfoClose"></a>
                    </div>
                    <div class="s-body">
                        <iframe id="ifrmSystemVersionInfo" style="width: 100%; height: 492px; background-color: #fff; margin-top: 10px"></iframe>
                ...
                <div id="reminder_triggered_Modal" class="modalDialog2 fade in" style="position: relative; height: 100%;">
                    ...
                    <div>
                        <div class="s-content" style="height: 21%; min-width: 550px;">
                            ...
                            <div class="s-body">
                                <div class="bootbox-body" style="color: #000; margin-top: 5px; margin-bottom: 5px;">
                                    <asp:Label runat="server" ID="lblReminderTriggered"></asp:Label>
                                </div>
                            </div>
                            <div class="s-footer">
 ...
 <!-- Sticky Notes Modal-->
        <div class="note-container" id="note_Modal" style="display: none; top: 35% !important; z-index: 2000;">
            <asp:UpdatePanel runat="server" ID="uplNoteBody" UpdateMode="Conditional">
                <ContentTemplate>
                    <div class="s-content note" id="note_parent_div" style="cursor: move; padding: 5px; border-style: solid; border-width: 1px; border-top: 0px; width: 447px;">
                        <div class="s-header" style="padding: 0; text-align: center;">
                            <h4 class="s-title" style="font-weight: bold;">Notes
                                <asp:LinkButton ID="note_close" OnClick="btnNoteCloseClick" runat="server"><i class="fa fa-close" style="float:right;cursor:pointer;margin-top: 10px;margin-right: 10px; color:#fff;"></i></asp:LinkButton>

                                <i style="width: 14px; height: 16px;"></i>

                                <i class="fa fa-window-maximize" onclick="expandNotes();" id="icoExpandNote" style="display: none; font-size: 16px; float: left; cursor: pointer; margin-top: 10px; margin-right: 10px; color: #fff;"></i>

                                <i style="width: 14px; height: 16px;"></i>

                                <i class="fa fa-window-restore" aria-hidden="true" onclick="contractNotes();" id="icoContractNote" style="float: left; cursor: pointer; margin-top: 10px; margin-right: 10px; color: #fff;"></i>
                            </h4>
                        </div>
	...
	<div>	
                ...
                <div class="navbar">
                    <div class="navbar-header">
                        ...
                        <a class="navbar-brand visible-lg visible-md">
                            ...
                            <span>
                                <span class="actura_cption">Actura <span>&#174;</span></span>
                            </span>
                        </a>
                        ...
                        <ul class="nav navbar-nav pull-right">
                            ...
                            <li class="dropdown">
                                ...
                                <ul class="dropdown-menu right" role="menu">
                                    ...
                                    <li><a href="#" id="btnSystemVersionInfo"><i style="font-size: 22px;" class="fa fa-info-circle"  aria-hidden="true"></i> Version Info.</a>
                                    </li>
                                    <li><a id="about" style="cursor: pointer;"><i class="fa fa-copyright"></i>About</a>
                                    </li>
                                    <li>
                                        <asp:LinkButton ID="logout" OnClick="btnLogout_Click" runat="server"><i class="fa fa-power-off"></i> Logout</asp:LinkButton>
                                    </li>
                                </ul>
                            </li>
                            <li style="float: left;">
                            ...
                            <li style="float: right;" id="openNotification" onclick="openNav()">
                                <a id="notification" href="#" title="Notification(s)" style="display: block !important;">
                                    <i class="fa fa-globe" style="font-size: 23px;"></i>
                                </a>
                                <asp:UpdatePanel runat="server" ID="UpdatePanel1" UpdateMode="Conditional">
                                    <ContentTemplate>
                                        <span class="notificationNumber" id="UnreadMessages" runat="server" style="display: none;"></span>
                                    </ContentTemplate>
                                    <Triggers>
                                        <asp:AsyncPostBackTrigger ControlID="tmrNotification" EventName="Tick" />
        ...
        <div id="notification_sideNav" class="sidenav" style="width: 0; padding-top: 0 !important;">
                            ...
                            <section>
                                <div class="tabs tabs-style-topline">
                                    ...
                                    <div>
                                        <nav>
                                            <ul>
                                                <li class="t_sort" style="border-top: none !important; border-bottom: 3px solid #248fbd !important;"><a class="source" style="float: right; padding-top: 0;"><i id="remove_notification" style="font-size: 14px; padding-right: 20px;" class="fa fa-trash"></i><i id="cancel_notification" style="font-size: 14px; padding-right: 10px; display: none;" class="fa fa-close"></i><i id="delete_notification" style="font-size: 14px; padding-right: 10px; display: none;" class="fa fa-check"></i><i id="back_notification" style="font-size: 14px; padding-right: 20px; display: none;" class="fa fa-arrow-circle-left"></i><i id="check_all" style="font-size: 14px; padding-right: 10px; display: none;" class="fa fa-check-circle-o"></i><i id="uncheck_all" style="font-size: 14px; padding-right: 10px; display: none;" class="fa fa-check-circle"></i></a></li>
                                    ...
                                    <ul id="sideNav" class="nav nav-pills nav-stacked notification_section" style="background: rgba(70,86,100,1);">
                                        <asp:UpdatePanel runat="server" ID="uplNotifications" UpdateMode="Conditional">
                                            <ContentTemplate>

                                                <asp:Repeater ID="rep_Notifications" runat="server" OnItemDataBound="itemBound">
                                                    <ItemTemplate>
                                                        <li id="drag" class="d e drag1">
                                                            <a href="#" class="notification_href" style="background: transparent; width: 91% !important;">
                                                                <i style="font-size: 14px;" class="fa fa-envelope"></i>
                                                                <span class="txt">
                                                                    <asp:Button CssClass="role_notification" Style="width: 90%; text-align: left; background: transparent; border: 0px; color: #F2F1EF;" ID="BtnUserNotification" runat="server"
                                                                        CommandName='<%# DataBinder.Eval(Container.DataItem,"Description") %>' UseSubmitBehavior="false" CommandArgument='<%# DataBinder.Eval(Container.DataItem,"Description") %>'
                                                                        Text='<%# DataBinder.Eval(Container.DataItem,"Description") %>' Enabled="true" ToolTip='<%# DataBinder.Eval(Container.DataItem,"Description") %>' OnClientClick="return false" />

                                                                </span>

                                                            </a>
                                                            <asp:CheckBox ID="chk_box" runat="server" Checked="false" CssClass="notification_checkbox" Style="display: none;" />


                                                            <div class="notificationStatus">
                                                                <asp:HiddenField ID="notification_status" runat="server" Value='<%# DataBinder.Eval(Container.DataItem,"Status") %>' />
                                                            </div>
                                                            <div class="notificationType">
                                                                <asp:HiddenField ID="notification_type" runat="server" Value='<%# DataBinder.Eval(Container.DataItem,"Type") %>' />
                                                            </div>
                                                            <div class="notificationID">
                                                                <asp:HiddenField ID="notification_id" runat="server" Value='<%# DataBinder.Eval(Container.DataItem,"RecID") %>' />
                                                            </div>
                                                            <div class="notificationMessage">
                                                                <asp:HiddenField ID="notification_message" runat="server" Value='<%# DataBinder.Eval(Container.DataItem,"Message") %>' />
                                                            </div>
                                                            <div class="notificationTaskCode">
                                                                <asp:HiddenField ID="notification_task" runat="server" Value='<%# DataBinder.Eval(Container.DataItem,"TaskCode") %>' />
                                                            </div>
                                                        </li>
                                                    </ItemTemplate>
                                                </asp:Repeater>                 
                                    ...
                                    <ul id="sideNav" class="nav nav-pills nav-stacked notification_message_section" style="background: rgba(70,86,100,1); display: none;">
                                        <li>
...
<asp:UpdatePanel ID="uplExpiredReminders" runat="server" style="display:none"><ContentTemplate>
    ...
    <table>
                    ...
                    <asp:Repeater ID = "rptExpiredReminders" runat="server" >
                        ...
                        <asp:UpdatePanel ID="uplExpiredReminders" runat="server" style="display: none;">
                            <ContentTemplate>
                                                    ...
                                                    <table>
                                                        ...
                                                        <asp:Repeater ID="rptExpiredReminders" runat="server">
                                                            <ItemTemplate>
                                                                <tr>
                                                                    <td>
                                                                        <asp:Label ID="lbltitle" Text='<%# Eval("ReminderID") %>' ToolTip='<%# "Expired On: </br> " + Eval("TriggerTime") %>' runat="server"></asp:Label></td>
                                                                    <td></td>
                                                                    <td style="padding-left: 2em;">
                                                                        <asp:Label ID="lblMessage" Text='<%# Eval("Message").ToString().Length>=20?Eval("Message").ToString().Substring(0,19):Eval("Message").ToString() %>' ToolTip='<%# Eval("Message") %>' runat="server"></asp:Label></td>
                                                                    <td></td>
            ...
            OnTick="reminderTriggered" Enabled="false">
                        ...
                        <div id="reminder_Modal" class="popup_col" style="display: none;">
                            <div style="position: relative; height: 100%; background-color: #fff; width: 94%; margin-left: 10px;">
                                <header>
                                    <div class="s-header" style="padding: 0; text-align: center;">
                                        <h4 class="s-title" style="font-weight: bold;">Reminders<div id="popup_col_close" class="fa fa-close" style="float: right; cursor: pointer; margin-top: 10px; margin-right: 10px;"></div>
                                        </h4>
                                    </div>
                                </header>
                                <asp:UpdatePanel runat="server" ID="uplReminders" UpdateMode="Conditional">
                        ...
                        <!--V2017-->
                        <asp:Timer ID="tmrNotification" runat="server" Interval="30000"
                            OnTick="tmrNotification_Tick" Enabled="true">
                        </asp:Timer>
                        <!-- V2007 -->
                            ...
                            <div style="position: relative; width: 98%; float: right; margin-top: 10px;">
                                <div class="wrapper gridster ready">
                                    <ul id="hello">
                                        ...
                                        <li id="li3" style="overflow: hidden; background-color: #2F8593;" class="gs-w container1" data-row="1" data-col="1" data-sizex="2" data-sizey="2">
                                            <header style="font-size: 14px; width: 100%; font-weight: normal; text-align: left; cursor: default;">Favorites</header>
                                        ...
                                        <li id="li5" class="gs-w bg-green" data-row="4" data-col="2" data-sizex="1" data-sizey="1">
                                            <p id="li5_date" style="margin-top: 40%; color: #fff; font-size: 5em;"></p>
                                            <p id="li5_day" class="widget_text" style="margin-top: 28%;"></p>
                                        </li>
                                        ...
                                        <li id="li7" class="gs-w" style="display: none; background-color: rgb(221, 221, 221);" data-row="3" data-col="1" data-sizex="2" data-sizey="1"></li>
                                        <li id="li8" class="gs-w bg-red" data-row="3" data-col="3" data-sizex="1" data-sizey="1">
                                            <header></header>
                                    ...
                                    <div class="slideshow" id="slideshow">
                                        <ol class="slides">
                                            <li class="current">
                                                ...
                                                <div class="description" style="width: 57%;">
                                                    <h2>X-ONT Software (Pvt) Ltd</h2>
                                                    <p style="font-style: italic;line-height: 20px;text-align: justify !important;font-variant: unset;font-size: 16px !important;font-weight: initial;">
                                                        X-ONT ACTURA Finance & Distribution ERP solution is a browser based enterprise-wide integrated application that addresses the business needs of dynamic companies. X-ONT ACTURA solutions strength is its ability to connect the distributed business processes into an integrated information management system that does everything a company expects to achieve more. The power of X-ONT ACTURA real-time integration supports and simplifies enterprise-wide business processes of small and medium scale companies, who rely on many internal and external business channels and service providers to achieve their business objectives, Its function and scope covers all accounting, distribution & selling aspects of a company. 
                                                    </p>
                                                </div>
                                                <div class="tiltview col" style="left: 30% !important; width: 70% !important; top: 50% !important;">
                                                    <a>
                                                        <img src="img/Tab1.jpg" /></a>
                                                </div>
                                            ...
                                            <li>
                                                <div class="description">
                                                    <h2>WHY X-ONT? </h2>
                                                    <p style="margin-top: 20px;" class="acturaPtext">
                                                        In essence X-ONT believes there are certain criteria that potential clients will be using in their selection of a software partner. 
                                                       
                                                        </p>
                                                    <p style="margin-top: 10px;" class="acturaPtext">
                                                        <br>
                                                         Financially Stable
                                                        <br>
                                                        ...
                                                         Proven Track Record
                                                        <br>
                                                         Thorough Knowledge of the Retail Industry
                                                        <br>
                                                         Modern Technology
                                                        <br>
                    ...
                    <div class="container" style="font-size: 10px !important; width: 100%; display: none; padding-left: 0; padding-right: 0;">
                        <div id="wrapper" style="width: 100%;">
                            <div style="float: left">
                                <%--V2022--%>
                                <ul id="crumb" style="font-size: 14px;color: #166f95; font-weight: bold;"></ul>
                                <a href="#" id="minimizeTabs" style="font-size: 19px; margin-top: -12px;top:0;position:absolute; display: none" class="btn" title="Minimize"><i class="fa fa-minus-square"></i></a>
                            </div>
    ...
    <asp:UpdatePanel ID="uplIframe4" runat="server"  UpdateMode="Conditional">
        <ContentTemplate>--%>
                            ...
                            <div id="tab_content5"> <%--V2038Added--%>
                                <!-- Tab content goes here -->
                            </div>
                            ...
                            <div id="help_Modal1" class="modalDialog2 fade in" style="display: none; position: fixed; height: 100%; width: 100%; margin: auto; top: 0; left: 0; right: 0; bottom: 0; z-index: 1000;">
                                <div>
                                    <div class="s-content" style="height: 90%; width: 80%;">
                                        <div class="s-header">
                                            <h4 class="s-title" style="font-weight: bold;">Help Menu </h4>
                                            <a style="font-size: 20px; float: right; text-decoration: none; cursor: pointer; margin-top: -15px;color:white;" class="fa fa-close" id="help_close1"></a>
                                        </div>
                                    ...
                                    <div class="s-content" style="height: 190px; width: 600px;">
                                        ...
                                        <div class="s-body">
                                            <form autocomplete="off">
                                                <br>
                                                Name:&nbsp;
                                                <input type="text" style="width: 50%;" class="keyup-char" id="fav_name" autocomplete="off" required /><span id="validation" style="margin-left: 5px; display: none; color: Red;">*</span>
                                                <br>
                                                URL:&nbsp;
                                                <input type="text" style="width: 80%; margin-left: 11px; text-overflow: ellipsis; overflow: hidden;" id="fav_value" readonly />
                                                <br>
                                                <br>
        ...
        <script src="./assets/js/common.js?v=2.0.22.0"></script>
        <!-- Slider -->
