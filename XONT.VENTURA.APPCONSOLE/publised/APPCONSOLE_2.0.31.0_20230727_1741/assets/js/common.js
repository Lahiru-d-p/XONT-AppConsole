...
function LoadTask(url, des, taskid, tasktype, username, applicationCode, exclusivityMode) {
    debugger;
    //V2036Adding start
    var localTaskType = url.indexOf('aspx') > 0 ? 'V2' : 'V3';

    if (window.event && window.event.ctrlKey)
        url = 'ComponentDiagnositics.aspx?TaskID=' + taskid + '&TaskType=' + localTaskType;
    //V2036Adding end
    ...
    //if (tab_count < 4) {V2038 Commented
    if (tab_count < 5) {//V2038 Added
        debugger;
        //V2049 S
        PageMethods.LogActiveTaskInfo(taskid, applicationCode, exclusivityMode,
        function (resultString) {
            if (resultString && resultString != '') {
                ...
                if ($("#" + taskid).length == 0) {
                    ...
                    $("#activeTask_tab .bootbox-body")[0].innerHTML = resultString;
                    $("#activeTask_tab").show();
                    $("#activeTask_ok").click(function () {
                        $("#activeTask_tab").fadeOut('slow', function (c) {
                        });
                    });
                    return;
                }
            ...
            //for (var current = 1; current < 5; current++) {V2038 Commented
            for (var current = 1; current < 6; current++) {//V2038 Added
            ...
            switch (tabToPopulate) {
                ...
                case 4:
                    ...
                    //V2038Adding start
                case 5:
                    addTab(url, des, taskid, tasktype, username, 5);
                    break;
                    //V2038Adding end
                default:
                    break;
        ...
        if ($("#" + taskid).length != 0) {
            //for (var l = 1; l < 5; l++) { V2038Removed
            for (var l = 1; l < 6; l++) { //V2038Added
                $("#tab_content" + l).hide();
            }
...
function addTab(url, des, taskid, tasktype, username, i) {
    ...
    if ($("#" + taskid).length != 0) {
        ...
        //for (var k = 1; k < 5; k++) {V2038Removed
        for (var k = 1; k < 6; k++) {//V2038Added
            $("#tab_content" + k).hide();
        }
        ...
        //V2045Adding start
        if (url.indexOf('ComponentDiagnositics') != -1) {
            task_path += ' (Version Info.)';
        }
        TaskText(task_path)
        //V2045Adding end

        //TaskText(task_path);V2036Removed
    ...
    $("#" + taskid + "_content").on('load', function () {
        ...
        var iframe = $("#" + taskid + "_content").contents(); //V2046
        iframe.find("body").addClass("bodybackgroundlogo"); //V2046
...
$(document).on('click', '#tabs a.remove', function (e) {
    ...
    $("#close_ok").click(function () {
        ...
        debugger;
        //PageMethods.ClearSessionTask(tabid); //V2049
        //cleanLocalStorage(tabid); //V2049
        PageMethods.ClearSessionTask(t); //V2049
        cleanLocalStorage(t);//V2049
...
$("#fav_save").click(function () {
    ...
    //var fav_role_name = $("span:contains(" + fav_taskcode + ")").parent().parent().parent().parent().prev().children().html();V2028Removed

    //V2028Adding start
    var closestLi = $("span:contains(" + fav_taskcode + ")").closest('li.d.view.hasSub');

    while (!closestLi.hasClass("role_name")) {
        closestLi = closestLi.prev();
    }

    fav_role_name = closestLi.children().html();
    //V2028Adding end
