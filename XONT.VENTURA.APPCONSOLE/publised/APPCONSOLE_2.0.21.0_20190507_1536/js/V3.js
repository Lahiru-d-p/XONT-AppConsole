...
$.ajax(GetAPI_Prefix() + '/api/CustomControl/GetInitialControlData?taskCode=' + currentTask, {
    success: function (data) {
        ...
        localStorage.setItem(currentTask + '_ComponentVersion', JSON.stringify(data.ComponentVersion));
        localStorage.setItem(currentTask + '_ShouldNotify', JSON.stringify(data.ShouldCreateNotification));
    },
    error: function (error) {
        console.log(error);
    }
