...
if (!pmtMstCntDta) {
    var url = window.location.href;
    var array = url.split('/');
    var apiPrefixForPromptRequest = (location.hostname === "localhost") ? '' : ('/' + array[3]);

    $.ajax(apiPrefixForPromptRequest + '/api/CustomControl/GetMasterControlData?taskCode=PROMPT', {
        success: function (data) {
...
function numbersOnly(evt, noOfRoundableDigits, noOfDecimalDigits) {
    var iKeyCode = (evt.which) ? evt.which : evt.keyCode;
